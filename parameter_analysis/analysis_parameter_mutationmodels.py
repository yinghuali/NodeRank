...
all_path_model_list = get_model_path(path_model_file)

all_path_model_list = sorted(all_path_model_list)
all_path_config_list = [i.replace('.pt', '.pkl') for i in all_path_model_list]
all_hidden_channel_list = [int(i.split('/')[-1].split('_')[2]) for i in all_path_config_list]


def get_filter_path(save_name):
    new_path_model_list = []
    new_path_config_list = []
    ...
    if save_name=='hidden20':
        for i in range(len(all_hidden_channel_list)):
            if all_hidden_channel_list[i]==20:
                ...
                new_hidden_channel_list.append(all_hidden_channel_list[i])
    if save_name=='heads2':
        for i in range(len(all_path_config_list)):
            dic = pickle.load(open(all_path_config_list[i], 'rb'))
            if 'heads' in dic:
                if int(dic['heads'])==2:
                    new_path_model_list.append(all_path_model_list[i])
                    new_path_config_list.append(all_path_config_list[i])
        ...
        for i in range(len(all_path_config_list)):
            ...
            if 'heads' in dic:
                if int(dic['heads'])==8:
                    ...
                    new_path_config_list.append(all_path_config_list[i])
                    new_hidden_channel_list.append(all_hidden_channel_list[i])
    if save_name=='slope01':
        for i in range(len(all_path_config_list)):
            dic = pickle.load(open(all_path_config_list[i], 'rb'))
            if 'negative_slope' in dic:
                if int(dic['negative_slope'])==0.1:
                    new_path_model_list.append(all_path_model_list[i])
                    new_path_config_list.append(all_path_config_list[i])
                    new_hidden_channel_list.append(all_hidden_channel_list[i])
    if save_name=='slope02':
        for i in range(len(all_path_config_list)):
            dic = pickle.load(open(all_path_config_list[i], 'rb'))
            if 'negative_slope' in dic:
                if int(dic['negative_slope'])==0.2:
                    new_path_model_list.append(all_path_model_list[i])
                    new_path_config_list.append(all_path_config_list[i])
                    new_hidden_channel_list.append(all_hidden_channel_list[i])
    if save_name=='slope03':
        for i in range(len(all_path_config_list)):
            dic = pickle.load(open(all_path_config_list[i], 'rb'))
            if 'negative_slope' in dic:
                if int(dic['negative_slope'])==0.3:
                    new_path_model_list.append(all_path_model_list[i])
                    new_path_config_list.append(all_path_config_list[i])
                    new_hidden_channel_list.append(all_hidden_channel_list[i])
    if save_name=='slope04':
        for i in range(len(all_path_config_list)):
            dic = pickle.load(open(all_path_config_list[i], 'rb'))
            if 'negative_slope' in dic:
                if int(dic['negative_slope'])==0.4:
                    new_path_model_list.append(all_path_model_list[i])
                    new_path_config_list.append(all_path_config_list[i])
                    new_hidden_channel_list.append(all_hidden_channel_list[i])
...
path_model_list, path_config_list, hidden_channel_list = get_filter_path(save_name)
